...
parser.add_argument('-lr', '--lr', type=float, help="", default=5e-2)
...
parser.add_argument('-RPU', '--RPU', type=str, help="", default='ReRamSB')
...
parser.add_argument(
    ...
    type=float,
    default=0.05,
    help='Reference-device d2d sigma used when symmetry-point subtraction is enabled.',
)
parser.add_argument(
    '--reference_mean',
    type=float,
    default=0.0,
    help='Reference-device mean offset; sweep this for non-zero reference-offset studies.',
)
parser.add_argument(
    '--subtract-symmetry-point',
    dest='subtract_symmetry_point',
    action='store_true',
    help='Store each device symmetry point on the reference device.',
)
parser.add_argument(
    '--no-subtract-symmetry-point',
    dest='subtract_symmetry_point',
    action='store_false',
    help='Disable symmetry-point subtraction onto the reference device.',
)
parser.set_defaults(subtract_symmetry_point=True)
parser.add_argument(
    "--block-type",
    type=str,
    nargs="+",
    default=None,
    help="6 tokens in {A,D}: conv1 layer1 layer2 layer3 layer4 fc. e.g. D D D D D A"
)
parser.add_argument(
    "--blocks",
    type=str,
    default=None,
    help="Alternative compact form, 6 letters, e.g. DDDDD A -> 'DDDDDA'"
)
...
valid_traing_mode = [
    ...
    'FULL_ANALOG', # load pretrained weights, then convert the whole model to analog
...
FULL_ANALOG_MODES = {'FULL_ANALOG', 'FULL_ANALOG_FS'}
...
def parse_blocks_type(args, default="DDDDDA"):
    """
    Return blocks_type_list length 6 for ResNet:
      [conv1, layer1, layer2, layer3, layer4, fc]
    Priority:
      1) --block-type (6 tokens)
      2) --blocks (string of length 6)
      3) default
    """
    if getattr(args, "block_type", None) is not None:
        bt = args.block_type
        assert len(bt) == 6, f"--block-type must have 6 tokens, got {len(bt)}"
        bt = [x.strip().upper() for x in bt]
    elif getattr(args, "blocks", None):
        s = args.blocks.strip().upper()
        assert len(s) == 6, f"--blocks must have length 6, got '{s}'"
        bt = list(s)
    else:
        bt = list(default)

    for x in bt:
        assert x in ("A", "D"), f"block type must be A or D, got {bt}"
    return bt

# Selective finetune keeps the historical default of an analog classifier only.
# Full-analog modes convert the entire model, so default block selection is all-analog.
default_bt = "AAAAAA" if train_mode in FULL_ANALOG_MODES else "DDDDDA"

blocks_type_list = parse_blocks_type(args, default=default_bt)
...
def get_opt_type(optimizer_str):
    ...
    analog_list = [
        'Analog SGD', 'TT-v1', 'TT-v2','RL-v2', 'TT-v3', 'TT-v4', 'mp'
    ]
...
train_idx = perm_train[: int(frac * n_train)].tolist()
trainset_small = Subset(trainset, train_idx)

n_test = len(testset)
perm_test = torch.randperm(n_test, generator=g)
test_idx = perm_test[: int(frac * n_test)].tolist()
testset_small = Subset(testset, test_idx)

trainloader = DataLoader(
    trainset_small, batch_size=BATCH_SIZE, shuffle=True, num_workers=6, pin_memory=True
)
testloader = DataLoader(
    testset_small, batch_size=128, shuffle=False, num_workers=6, pin_memory=True
)
...
def get_RPU_device(device_name=None):
    ...
    if device_name == 'CS':
        ...
        return SoftBoundsReferenceDevice(
            ...
            subtract_symmetry_point=args.subtract_symmetry_point,
        ...
        device=SoftBoundsReferenceDevice(
            ...
            corrupt_devices_prob = 0.0,    # 0.0977
...
def _to_serializable(x):
    ...
    # objects (AIHWKIT configs typically end up here)
...
def get_config(config_name):
    if config_name == 'FP SGD':
        ...
        rpu_config.device.transfer_lr = 0.01
        rpu_config.device.scale_transfer_lr = True
...
def set_pref_from_config(rpu_config, enable: bool):
    if enable:
        ...
        print("[PY] Pref OFF")
def convert_resnet_blocks_to_analog(model, rpu_config, blocks_type_list, exclude_modules=None):
    """
    blocks_type_list: length 6 list, each in {'A','D'} for:
      [conv1, layer1, layer2, layer3, layer4, fc]
    """
    ...
    if blocks_type_list[0] == 'A':
        model.conv1 = convert_to_analog(model.conv1, rpu_config=rpu_config, exclude_modules=exclude_modules)
    if blocks_type_list[1] == 'A':
        model.layer1 = convert_to_analog(model.layer1, rpu_config=rpu_config, exclude_modules=exclude_modules)
    if blocks_type_list[2] == 'A':
        model.layer2 = convert_to_analog(model.layer2, rpu_config=rpu_config, exclude_modules=exclude_modules)
    if blocks_type_list[3] == 'A':
        model.layer3 = convert_to_analog(model.layer3, rpu_config=rpu_config, exclude_modules=exclude_modules)
    if blocks_type_list[4] == 'A':
        model.layer4 = convert_to_analog(model.layer4, rpu_config=rpu_config, exclude_modules=exclude_modules)
    if blocks_type_list[5] == 'A':
        model.fc = convert_to_analog(model.fc, rpu_config=rpu_config, exclude_modules=exclude_modules)

    return model

def train():
    use_pretrained_weights = train_mode != 'FULL_ANALOG_FS'
    ...
    if MODEL_NAME == 'Resnet18':
        ...
        model = models.resnet34(
            weights=models.ResNet34_Weights.IMAGENET1K_V1 if use_pretrained_weights else None
        )
    elif MODEL_NAME == 'Resnet50':
        model = models.resnet50(
            weights=models.ResNet50_Weights.IMAGENET1K_V1 if use_pretrained_weights else None
        )
    elif MODEL_NAME == 'Resnet101':
        model = models.resnet101(
            weights=models.ResNet101_Weights.IMAGENET1K_V1 if use_pretrained_weights else None
        )
    elif MODEL_NAME == 'Resnet152':
        model = models.resnet152(
            weights=models.ResNet152_Weights.IMAGENET1K_V1 if use_pretrained_weights else None
        )
    ...
    #     model = models.mobilenet_wd4(pretrained=True)
        ...
        model = models.mobilenet_v3_large(
            weights=models.MobileNet_V3_Large_Weights.IMAGENET1K_V1 if use_pretrained_weights else None
        )
    elif MODEL_NAME == 'MobileNetV3S':
        model = models.mobilenet_v3_small(
            weights=models.MobileNet_V3_Small_Weights.IMAGENET1K_V1 if use_pretrained_weights else None
        )
    else:
        raise ValueError(f"unknown model: {MODEL_NAME}")
    print(f"[Model Init] {'ImageNet pretrained' if use_pretrained_weights else 'random init'}")
    ...
    train_stage = 'finetune' if use_pretrained_weights else 'scratch'
    dataset_name = f'{args.dataset}-{train_stage}/{MODEL_NAME}/{train_mode}'
    ...
    if opt_type is opt_T.TORCH:
        ...
        assert train_mode not in FULL_ANALOG_MODES
    else:
        # if USE_CUDA:
        #     model.cuda(DEVICE)
        ...
        if opt_type is opt_T.KIT_ANALOG:
            alg_name += f'-tau={tau}'
            RPU_suffix = args.RPU
            if args.RPU in {'Softbounds', 'HfO2', 'OM', 'ReRamSB'}:
                RPU_suffix += f'-symm={"on" if args.subtract_symmetry_point else "off"}'
                RPU_suffix += f'-rmean={args.reference_mean:g}'
                RPU_suffix += f'-rstd={args.reference_std:g}'
        ...
        # replace the last layer
        if train_mode in FULL_ANALOG_MODES:
            replaced_layer = nn.Linear(num_ftrs, num_classes)
        else:
    ...
    if MODEL_NAME[:6] == 'Resnet':
        ...
        if (opt_type is opt_T.KIT_ANALOG) and (train_mode not in FULL_ANALOG_MODES):
            exclude_modules = []
            model = convert_resnet_blocks_to_analog(
                model=model,
                rpu_config=rpu_config,
                blocks_type_list=blocks_type_list,
                exclude_modules=exclude_modules,
            )
            print(f"[BlockType] Applied: {''.join(blocks_type_list)}  (conv1 l1 l2 l3 l4 fc)")
    elif MODEL_NAME[:9] == 'MobileNet':
    ...
    text_log_path = logger.get_text_log_path()  # e.g., ".../events.out.tfevents....log"
    ...
    save_run_config_json(
        ...
        extra={
            ...
            "alg_name": alg_name,
            "RPU_NAME": RPU_NAME,
            "num_of_states": num_of_states if "num_of_states" in globals() else None,
            "tau": tau if "tau" in globals() else None,
            "args": vars(args) if "args" in globals() else None,
        },
    ...
    # scheduler = StepLR(optimizer, step_size=30, gamma=0.85)
    allstart = time.time()
