...
parser.add_argument('--tail_weightening', type=float, default=10)
parser.add_argument('-RPU', '--RPU', type=str, help="", default='HfO2')
...
# global optimizer LR (AnalogSGD lr)
...
# TT-v2 learning rates
parser.add_argument('--fast_lr_tt', type=float, default=0.01)
parser.add_argument('--transfer_lr_tt', type=float, default=0.5)

# scale_transfer_lr for TT-v2 (0/1 to avoid store_true ambiguity in sweeps)
parser.add_argument('--scale_transfer_lr_tt', type=int, default=1)  # 1=True, 0=False

# for reproducibility (if you don’t already have it)
...
def get_opt_type(optimizer_str):
    ...
    analog_list = [
        'Analog SGD', 'TT-v1', 'TT-v2', 'TT-v3', 'TT-v4', 'mp', 'RL-v2', 'RL-v3'
    ]
...
def get_config(config_name):
    if config_name == 'FP SGD':
        ...
        config = {
            'name': 'TT-v2',
            'rpu_config': rpu_config,
            'optimizer_cls': get_AnalogSGD_optimizer_generator(lr=args.LR),
            'grad_per_iter': 1,
            'batch_size': BATCH_SIZE,
        }
        ...
        rpu_config.mapping.learn_out_scaling = True
        rpu_config.mapping.weight_scaling_columnwise = True
        rpu_config.device.tail_weightening = args.tail_weightening
        rpu_config.mapping.weight_scaling_omega = 0.3
        ...
        if num_of_states == 64:
            ...
            rpu_config.device.transfer_lr = 0.02 
...
def set_pref_from_config(rpu_config, enable: bool):
    if enable:
        ...
        os.environ["AIHWKIT_PREF_GAMMA"] = str(float(args.gamma))
...
# Used to enable/disable the residual part chopper*gamma*(P-Q)
...
if os.environ.get("AIHWKIT_PREF_ON") == "1":
    alg_name += f'-pref-gamma={args.gamma:g}'
if setting == 'RL-v2':
    alg_name += f'-tw={args.tail_weightening:g}'
if opt_type is opt_T.KIT_ANALOG:
    alg_name += f'-tau={tau}'
    if args.SB_dw_min_std > 0:
