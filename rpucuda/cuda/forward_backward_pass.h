...
#pragma once
#if defined(RPU_DEBUG_CHOPPER_CHECK) && !defined(CHOPPER_PREF_ON)
#define CHOPPER_PREF_ON
#endif
...
public:
  ...
  // Instance-level pref gating (default disabled).

  void setPref(const T *dev_pref, T gamma, bool apply_fwd = true, bool apply_bwd = true) {
    dev_pref_ = dev_pref;
    pref_gamma_ = gamma;
    pref_apply_fwd_ = apply_fwd;
    pref_apply_bwd_ = apply_bwd;
  }

  void clearPref() {
    dev_pref_ = nullptr;
    pref_gamma_ = (T)0;
    pref_apply_fwd_ = true;
    pref_apply_bwd_ = true;
    
  }
  inline bool prefEnabled(bool transposed) const {
    if (dev_pref_ == nullptr || pref_gamma_ == (T)0) {
      return false;
    }
    return transposed ? pref_apply_bwd_ : pref_apply_fwd_;
  }
...
private:
  ...
  // --- pref state (default disabled) ---
  const T *dev_pref_ = nullptr;
  T pref_gamma_ = (T)0;
  bool pref_apply_fwd_ = true;
  bool pref_apply_bwd_ = true;
