...
case AnalogMVType::OnePass: {
  // Pref enabled only for this instance and direction.
  if (this->prefEnabled(transposed)) {
    const int m_batch = iom.getMBatch();
    const int in_size = iom.getInSize();
    const int out_size = iom.getOutSize();
    const int total_in_size = m_batch * in_size;
    const int total_out_size = m_batch * out_size;

    // Reuse shared buffers (OnePass normally doesn't use these IDs).
    // We will reuse POS_NEG_IN buffer first as input backup, then as main output buffer.
    T *pref_in_backup = context_->template getSharedBuffer<T>(
        RPU_BUFFER_POS_NEG_IN, MAX(total_in_size, total_out_size));
    T *pref_out = context_->template getSharedBuffer<T>(
        RPU_BUFFER_POS_NEG_OUT, MAX(total_in_size, total_out_size));

    // 1) Backup input because computeAnalogMVSinglePass may overwrite inBuffer (output noise paths).
    RPU::math::copyWithIterator(context_, pref_in_backup, iom.getInBuffer(), total_in_size);

    // 2) Pref pass on cpq_out = c*(P-Q). Signature wants T*, so cast away const.
    computeAnalogMVSinglePass(
        const_cast<T *>(this->dev_pref_), iom, mv_pars, out_trans, transposed);

    // 3) Finalize pref output, but DO NOT engage bound-management (force applyToOutput(..., false)).
    {
      auto io = iom.getIO();
      if (io.hasNLCalibration()) {
      ...
      // Always false: pref pass should not affect bound-test loop semantics.
      iom.applyToOutput(pref_out, out_trans, false);
    }

    // 4) Restore input for main pass.
    RPU::math::copyWithIterator(
        context_, const_cast<T *>(iom.getInBuffer()), pref_in_backup, total_in_size);

    // After restoring input, we can reuse pref_in_backup as the main-output buffer.
    T *main_out = pref_in_backup;

    // 5) Main pass on W, but finalize into main_out (T*) so we can combine with pref_out.
    computeAnalogMVSinglePass(dev_weights, iom, mv_pars, out_trans, transposed);
    bool bound_success = finalizeOutput(main_out, iom, mv_pars, out_trans, transposed);

    // 6) Digital subtraction: out = main_out + gamma * pref_out
    RPU::math::elemweightedsum(
        context_,
        pref_out, total_out_size,
        pref_out, (T)(this->pref_gamma_),
        pref_out, (T)0.0);

    // out_values = main_out + pref_out
    RPU::math::addWithIterator(context_, out_values, main_out, pref_out, total_out_size);
  ...
  // Default (no pref): original behavior.
  computeAnalogMVSinglePass(dev_weights, iom, mv_pars, out_trans, transposed);
  return finalizeOutput(out_values, iom, mv_pars, out_trans, transposed);
}
