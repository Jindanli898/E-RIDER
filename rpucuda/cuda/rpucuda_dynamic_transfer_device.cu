  ...
  RPU_CUDA_1D_KERNEL_LOOP(idx, w_size) {
    ...
    for (int i_rep = 0; i_rep < n_repeats; i_rep++) {
      ...
      // dw = (in_chop != out_chop) ? -dw : dw;
            // --- compute raw c*(P - Qref) (no lr_scale, no accumulation correction) ---
      T dw_raw = (val - past_mean_w);
      dw_raw = (in_chop != out_chop) ? -dw_raw : dw_raw;

      // Store matrix element. For full read, require n_repeats==1; otherwise guard.
      if (cpq_out != nullptr && i_rep == 0) {
        cpq_out[buffer_idx] = dw_raw;  // out_size-major, aligned with buffer_weight/past_mean_weight
      }
      T dw = dw_raw;
...
void DynamicTransferRPUDeviceCuda<T>::readAndUpdate(
  ...
  RPU_GET_CUDA_BUFFER(this->context_, T, dev_cpq_out_, w_size);
  if (n_vec != in_size) {
    DEBUG_OUT("n_vec=" << n_vec << " in_size=" << in_size << " out_size=" << out_size);

    // RPU_FATAL("kernelDynamicTransfer full-matrix export requires m_batch == in_size.");
}
  ...
  detail::kernelDynamicTransfer<T><<<nblocks, nthreads, 0, this->context_->getStream()>>>(
      ...
      par.experimental_fast_lr_feedback ? dev_feedback_->getData() : nullptr, dev_cpq_out_->getData());
  constexpr int print_every = 100000;  
  if (this->current_update_idx_ % print_every == 0) {
    this->debugDumpRunningMeanStatsToFile("mean_w_stats.csv");
  }
...
template <typename T>
void DynamicTransferRPUDeviceCuda<T>::debugPrintRunningMean() {
  const auto &par = getPar();
  int in_size  = par.getInSize();
  int out_size = par.getOutSize();
  int w_size   = in_size * out_size;

  DEBUG_OUT("Running mean weight:");
  DEBUG_CALL(
      CudaArray<T> dev_a(this->context_, w_size);
      math::copyWithIterator(
          this->context_, dev_a.getData(), running_mean_weight, w_size);
      this->context_->synchronize();
      dev_a.printMatrixValues(out_size);  
  );
}
// template <typename T>
// void DynamicTransferRPUDeviceCuda<T>::debugDumpRunningMeanToFile(const char *filename) {
//   const auto &par = getPar();
//   int in_size  = par.getInSize();
//   int out_size = par.getOutSize();
//   int w_size   = in_size * out_size;

//   std::vector<T> host_mean(w_size);

//   // 确保 kernel 已经跑完（尤其是在同一个 stream 上）
//   this->context_->synchronize();

//   cudaError_t err = cudaMemcpy(
//       host_mean.data(),                    // dst
//       dev_running_mean_->getData(),        // src: 这里改成真正的 buffer 指针
//       w_size * sizeof(T),
//       cudaMemcpyDeviceToHost);

//   if (err != cudaSuccess) {
//     std::cerr << "[debugDumpRunningMeanToFile] cudaMemcpy failed: "
...
//   if (!ofs) {
//     std::cerr << "[debugDumpRunningMeanToFile] Cannot open file "
//               << filename << " for writing." << std::endl;
//     return;
//   }

//   ofs << "==== step " << this->current_update_idx_ << " ====\n";

//   for (int i = 0; i < in_size; ++i) {
//     for (int j = 0; j < out_size; ++j) {
//       ofs << host_mean[i * out_size + j];
//       if (j + 1 < out_size) {
//         ofs << ' ';
//       }
//     }
...
// }
template <typename T>
void DynamicTransferRPUDeviceCuda<T>::debugDumpRunningMeanStatsToFile(const char *filename) {
  const auto &par = getPar();
  int in_size  = par.getInSize();
  int out_size = par.getOutSize();
  int w_size   = in_size * out_size;
  ...
  cudaError_t err = cudaMemcpy(
      host_mean.data(),
      dev_running_mean_->getData(),
      w_size * sizeof(T),
      cudaMemcpyDeviceToHost);

  if (err != cudaSuccess) {
    std::cerr << "[debugDumpRunningMeanStatsToFile] cudaMemcpy failed: "
              << cudaGetErrorString(err) << std::endl;
    return;
  }
  ...
  for (int k = 0; k < w_size; ++k) {
    double v = static_cast<double>(host_mean[k]);
    sum   += v;
    sumsq += v * v;
  }

  double mean = sum / (double)w_size;
  double var  = sumsq / (double)w_size - mean * mean;
  if (var < 0.0) {
    // 数值误差导致的轻微负数，截掉
    var = 0.0;
  }
  ...
  std::ofstream ofs(filename, std::ios::app);
  if (!ofs) {
    std::cerr << "[debugDumpRunningMeanStatsToFile] Cannot open file "
              << filename << " for writing." << std::endl;
    return;
  }

  // 建议写成一行 CSV：step, in_size, out_size, mean, std
  ofs << this->current_update_idx_ << ","
      << in_size << ","
      << out_size << ","
      << std::setprecision(10) << mean << ","
      << std::setprecision(10) << std  << "\n";
