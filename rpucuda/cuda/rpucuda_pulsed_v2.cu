 ...
 */
// KFAC has gap between using FPupdate and rankupdate, add debug on real and ideal weight update value statics 
...
auto env_int = [](const char *name, int defval) -> int {
  ...
  if (use_kfac) {
    ...
    static uint64_t kfac_debug_tile_counter = 0;
const uint64_t this_tile_id = kfac_debug_tile_counter++;
...
//   }
    ...
    const bool do_gnorm_rescale = env_on("AIHWKIT_KFAC_GNORM_RESCALE");
...
if (do_gnorm_rescale) {
  ...
  fflush(stderr);
    ...
    std::vector<T> hx_raw(nx), hx_pre(nx), hd_raw(nd), hd_pre(nd);
    cuda_copy_d2h(s, x_raw, hx_raw.data(), nx);
    cuda_copy_d2h(s, x_pre, hx_pre.data(), nx);
    cuda_copy_d2h(s, d_raw, hd_raw.data(), nd);
    cuda_copy_d2h(s, d_pre, hd_pre.data(), nd);

    double xrn = 0.0, xpn = 0.0, drn = 0.0, dpn = 0.0;
    double xmax_raw = 0.0, xmax_pre = 0.0, dmax_raw = 0.0, dmax_pre = 0.0;
    int d_nnz_raw = 0, d_nnz_pre = 0;

    for (int i = 0; i < nx; ++i) {
      double xr = (double)hx_raw[i];
      double xp = (double)hx_pre[i];
      xrn += xr * xr;
      xpn += xp * xp;
      xmax_raw = std::max(xmax_raw, std::abs(xr));
      xmax_pre = std::max(xmax_pre, std::abs(xp));
    }

    for (int i = 0; i < nd; ++i) {
      double dr = (double)hd_raw[i];
      double dp = (double)hd_pre[i];
      drn += dr * dr;
      dpn += dp * dp;
      dmax_raw = std::max(dmax_raw, std::abs(dr));
      dmax_pre = std::max(dmax_pre, std::abs(dp));
      if (dr != 0.0) d_nnz_raw++;
      if (dp != 0.0) d_nnz_pre++;
    }

    fprintf(stderr,
            "[KFAC-NORM] x_raw=%.3e x_pre=%.3e d_raw=%.3e d_pre=%.3e\n",
            std::sqrt(xrn), std::sqrt(xpn), std::sqrt(drn), std::sqrt(dpn));
    fprintf(stderr,
            "[KFAC-MAX ] x_raw=%.3e x_pre=%.3e d_raw=%.3e d_pre=%.3e\n",
            xmax_raw, xmax_pre, dmax_raw, dmax_pre);
    fprintf(stderr,
            "[KFAC-NNZ ] d_raw=%d/%d d_pre=%d/%d\n",
            d_nnz_raw, nd, d_nnz_pre, nd);
    fflush(stderr);
  }
    ...
    auto emit_dw_diag = [&](const char *mode, T lr) {
      ...
      fprintf(
          stderr,
          "[KFAC-DW] step=%llu tile_id=%llu mode=%s m_batch=%d x_size=%d d_size=%d "
