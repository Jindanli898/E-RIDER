...
public:
  const T *getPastMeanDataConst() const {
    return dev_past_mean_ ? dev_past_mean_->getDataConst() : nullptr;
  }
  const T *getCPQDataConst() const {
    return dev_cpq_out_ ? dev_cpq_out_->getDataConst() : nullptr;
  }
  ...
  swap(DynamicTransferRPUDeviceCuda<T> &a, DynamicTransferRPUDeviceCuda<T> &b) noexcept {
    ...
    swap(a.dev_cpq_out_, b.dev_cpq_out_);
  ...
  void debugPrintRunningMean();
  void debugDumpRunningMeanStatsToFile(const char *filename);

  // void debugDumpRunningMeanToFile(const char *filename);
...
private:
  ...
  std::unique_ptr<CudaArray<T>> dev_cpq_out_ = nullptr;
